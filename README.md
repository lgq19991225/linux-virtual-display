...
```bash
# install (auto-detect connector, default 1920x1080@60)
...
1. `generate-edid.py` creates a fake EDID for the desired resolution
2. `setup-virtual-monitor.sh` installs it to `/usr/lib/firmware/edid/` and adds `drm.edid_firmware` + `video=` parameters to GRUB
3. The trailing `e` in `video=<connector>:<WxH>@<Hz>e` forces the port to appear connected
